...
import SlackForm from "@/pages/RegistrationCenter/AlertManage/AlertInstance/components/SlackForm";
  ...
  return (
    ...
    >{
      ...
      {(values?.type == ALERT_TYPE.DINGTALK || alertType == ALERT_TYPE.DINGTALK) ?
        ...
        /> : undefined
      } {(values?.type == ALERT_TYPE.SLACK || alertType == ALERT_TYPE.SLACK) ?
        <SlackForm
          onCancel={() => {
            setAlertType(undefined);
            handleChooseModalVisible();
          }}
          modalVisible={values?.type == ALERT_TYPE.SLACK || alertType == ALERT_TYPE.SLACK}
          values={values}
          onSubmit={(value) => {
            onSubmit(value);
          }}
